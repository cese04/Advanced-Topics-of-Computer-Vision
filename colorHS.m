...
[x,y,z]=size(fon);
figure, subplot(221),imshow(hsv2rgb(fon))
subplot(222),imshow(hsv2rgb(Ima))
...
for i=1:x
    for j=1:y
        ...
        s=(S(i,j)-fS(i,j))^2;
        d=sqrt(h+s);
        if d>0.8  %If the pixel's distance is greater than the threshold is part of the object
            mask(i,j)=1;
        end
    end
end
